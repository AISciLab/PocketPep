...
def create_parser():
    ...
    argparser_mpnn.add_argument("--ca_only", action="store_true", default=False,
    ...
    argparser_mpnn.add_argument("--model_name", type=str, default="v_48_020",
                                help="ProteinMPNN model name: v_48_002, v_48_010, v_48_020, v_48_030; v_48_010=version with 48 edges 0.10A noise")
    argparser_mpnn.add_argument("--use_soluble_model", action="store_true", default=False,
    ...
    argparser_mpnn.add_argument("--save_score", type=int, default=0,
                                help="0 for False, 1 for True; save score=-log_prob to npy files")
    argparser_mpnn.add_argument("--save_probs", type=int, default=0,
                                help="0 for False, 1 for True; save MPNN predicted probabilites per position")
    argparser_mpnn.add_argument("--score_only", type=int, default=0,
                                help="0 for False, 1 for True; score input backbone-sequence pairs")
    argparser_mpnn.add_argument("--path_to_fasta", type=str, default="",
    ...
    argparser_mpnn.add_argument("--sampling_temp", type=str, default="0.1",
    ...
    argparser_mpnn.add_argument("--out_folder", type=str, default='tmp',
    ...
    argparser_mpnn.add_argument("--pdb_path_chains", type=str, default='',
                                help="Define which chains need to be designed for a single PDB ")
    argparser_mpnn.add_argument("--jsonl_path", type=str, default='tmp/chain_set_test.jsonl',
                                help="Path to a folder with parsed pdb into jsonl")
    argparser_mpnn.add_argument("--chain_id_jsonl", type=str, default='',
    ...
    argparser_mpnn.add_argument("--fixed_positions_jsonl", type=str, default='',
                                help="Path to a dictionary with fixed positions")
    argparser_mpnn.add_argument("--omit_AAs", type=list, default='X',
                                help="Specify which amino acids should be omitted in the generated sequence, e.g. 'AC' would omit alanine and cystine.")
    argparser_mpnn.add_argument("--bias_AA_jsonl", type=str, default='',
    ...
    argparser_mpnn.add_argument("--omit_AA_jsonl", type=str, default='',
    ...
    argparser_mpnn.add_argument("--pssm_multi", type=float, default=0.0,
                                help="A value between [0.0, 1.0], 0.0 means do not use pssm, 1.0 ignore MPNN predictions")
    argparser_mpnn.add_argument("--pssm_threshold", type=float, default=0.0,
    ...
    argparser_mpnn.add_argument("--tied_positions_jsonl", type=str, default='',
...
def process_mpnn_embedding_fn(sample):
    ...
    with torch.no_grad():
        ...
        mpnn_emb14 = model_4(X, S, mask, chain_M * chain_M_pos, residue_idx, chain_encoding_all, randn_1)

    mpnn_emb1 = mpnn_emb14.mean(dim=2)

    sample["mpnn_emb"] = mpnn_emb1.cpu()
...
model_4 = load_model(torch.load('vanilla_model_weights/v_48_030.pt'), ca=False)



def main(args):
        output_file = os.path.join("tmp", f"tmp.pdb")
        if os.path.exists(args.pdb_path):
            with open(args.pdb_path, "r") as inputFile, open(output_file, "w") as outFile:
            ...
            print('pdb is not exist')
            return

        if(flag==False):
            print('pdb Atom not exist')
            return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pdb-path", type=str, default="../../example/pocket.pdb",help="Path to the PDB file from which structural features will be extracted.")
    parser.add_argument("--out-path", type=str, default="../../example/pocket.pkl",help="Path to the output PKL file where the extracted features will be saved.")
